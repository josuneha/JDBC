...
public class OrderData {
	...
	OrderData(int orderId, String customerId, int companyName, Date orderDate) {
		this.orderId = orderId;
		this.customerId = customerId;
		this.employeeId = companyName;
		this.orderDate = orderDate;
	}

	OrderData(int orderId, String customerId, Date orderDate) {
		this.orderId = orderId;
		this.customerId = customerId;
		this.orderDate = orderDate;
	}
	...
	OrderData(String cid, int employeeId, Date oDate) {
		this.customerId = cid;
	...
	public static int insertOrder(Connection connection, OrderData order) {

		// String sql ="INSERT INTO Orders (OrderID, CustomerID, orderDate) "
		// + "VALUES (?, ?, ?)";
		...
		try {
			...
			stmtUpdate.setString(2, order.customerId);
			// stmtUpdate.setDate(3,order.orderDate);
			n = stmtUpdate.executeUpdate();
			stmtUpdate.close();
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println("Error in insert Order testej2: " + sql + " Exception: " + e);
		}
		return n;
	}

	// ------------------------------------------------------
	public static int insertOrder2(Connection connection, OrderData order) {

		String sql = "INSERT INTO Orders (CustomerID, orderDate, EmployeeID) " + "VALUES (?, ?, ?)";

		// String sql ="INSERT INTO Orders (OrderID, CustomerID) "
		// + "VALUES (?, ?)";
		...
		try {
			...
			stmtUpdate.setInt(3, order.employeeId);
			stmtUpdate.executeUpdate();
			ResultSet rs = stmtUpdate.getGeneratedKeys();
			if (rs != null && rs.next()) {
				n = rs.getInt(1);
			}
			stmtUpdate.close();
			...
			e.printStackTrace();
			System.out.println("Error in insert Order testej3: " + sql + " Exception: " + e);
		}
		return n;
	}
	...
	public static Vector<OrderData> getOrderList(Connection connection) {
		...
		try {
			...
			while (result.next()) {
				OrderData order = new OrderData(Integer.parseInt(result.getString("OrderID")),
						result.getString("CustomerId"), null, result.getDate("OrderDate"));
	...
	public static Vector<OrderData> getOrderListXCustomer(Connection connection, String id) {
		Vector<OrderData> vec = new Vector<OrderData>();

		String sql = "Select OrderId, OrderDate, EmployeeId FROM Orders, Customers";
		sql += " WHERE Orders.CustomerID = Customers.CustomerID AND Orders.CustomerID=?";
	...
	public static OrderData getOrderHeader(Connection connection, int id) {
		...
		try {
			...
			ResultSet result = pstmt.executeQuery();
			if (result.next()) {
				order = new OrderData(Integer.parseInt(result.getString("OrderID")), result.getString("CustomerId"),
						null, result.getDate("OrderDate"));
			}
			result.close();
			pstmt.close();
		} catch (SQLException e) {
	...
	public static Vector<OrderDetailData> getOrderDetail(Connection connection, int id) {
		...
		try {
			...
			while (result.next()) {
				OrderDetailData orderDetail = new OrderDetailData(Integer.parseInt(result.getString("OrderID")),
						result.getString("ProductID"), result.getString("ProductName"),
						Float.parseFloat(result.getString("UnitPrice")), Float.parseFloat(result.getString("Quantity")),
	...
	public static Vector<OrderDetailData> getSimpleOrderDetail(Connection connection, int id) {
		...
		try {
			...
			while (result.next()) {
				OrderDetailData orderDetail = new OrderDetailData(Integer.parseInt(result.getString("OrderID")),
						result.getString("ProductID"), "", Float.parseFloat(result.getString("UnitPrice")),
						Float.parseFloat(result.getString("Quantity")), Float.parseFloat(result.getString("Discount")));
				vec.addElement(orderDetail);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public static int updateUnitPriceOrderDetail(Connection connection, OrderDetailData orderDetail) {
		String sql = "UPDATE [Order Details] " + "SET UnitPrice = ?" + " WHERE OrderID = ?";
...
class OrderDetailData {
	...
	OrderDetailData(int orderId, String productId, String productName, float unitPrice, float quantity,
			float discount) {
		this.orderId = orderId;
