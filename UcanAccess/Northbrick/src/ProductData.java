...
public class ProductData {
	...
	public static Vector<ProductData> getProductList(Connection connection) {
		...
		try {
			...
			while (result.next()) {
				ProductData product = new ProductData(result.getString("ProductId"), result.getString("ProductName"),
						Integer.parseInt(result.getString("SupplierId")), result.getString("CompanyName"),
	...
	public static ProductData getProduct(Connection connection, String id) {
		...
		try {
			...
			result = pstmt.executeQuery();
			if (result.next()) {
				product = new ProductData(result.getString("ProductId"), result.getString("ProductName"),
						Integer.parseInt(result.getString("SupplierId")), null,
						Float.parseFloat(result.getString("UnitPrice")));
			}
			result.close();
			pstmt.close();
	...
	public static int updateProduct(Connection connection, ProductData product) {
		String sql = "UPDATE Products " + "SET ProductName = ?, SupplierId = ?, UnitPrice = ?" + " WHERE ProductId = ?";
